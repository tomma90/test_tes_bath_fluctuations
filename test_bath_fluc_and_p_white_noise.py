...
sigma_P = nep_aW_over_rthz * np.sqrt(sampling_rate_hz) / 1e18

fknee_hz = 0.1
alpha = 2
psd_Tb_K_over_rthz = 1.e-6 # just to try
sigma_Tb = psd_Tb_K_over_rthz * np.sqrt(sampling_rate_hz)
...
Tb = Tb + noise.add_pink_noise(fknee_hz, alpha, sigma_Tb, sampling_rate_hz, Tb)
P = P + noise.add_white_noise(sigma_P, P)
...
plt.figure()
plt.title('Detector Input', fontsize=20)
plt.plot(t[10000:], Tb[10000:], label = 'Bath Temperature')
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.xlabel('time [s]', fontsize=20)
plt.ylabel('Tbath [K]', fontsize=20)
plt.legend(fontsize=12, loc=3)
plt.tight_layout()
'''
plt.figure()
...
dTb = Tb[10000:]-np.mean(Tb[10000:])

length = len(t)

dI_fft = sp.fft.fft(dI)
dP_fft = sp.fft.fft(dP)
dTb_fft = sp.fft.fft(dTb)
...
dTb_psd = dTb_fft[0:length//2]*np.conjugate(dTb_fft[0:length//2])/sampling_rate_hz/length
...
plt.figure()
plt.title('Thermal Fluctuations', fontsize=20)
plt.loglog(freqs, dTb_psd**(1/2), label='bath thermal fluct')
plt.loglog(freqs, psd_Tb_K_over_rthz*np.ones_like(freqs), 'k--', label = '1 uK/sqrt(Hz)')
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.xlabel('frq [Hz]', fontsize=20)
plt.ylabel('PSD [K/sqrt(Hz)]', fontsize=20)
plt.legend(fontsize=12, loc=3)
plt.tight_layout()
'''
plt.show()
